...
- **Capabilities**:
  ...
  - Default Free tier provider: NVIDIA NIM (`meta/llama-3.2-11b-vision-instruct`, verified production default, with fallback to `nvidia/nemotron-3-super-120b-a12b`, `openai/gpt-oss-20b`, and `meta/muse-glimmer-30b`).
  - Free tier fallback provider: OpenAI (`gpt-4o-mini`, via `FREE_TIER_FALLBACK_PROVIDER_ID`).
  - Default Pro tier provider: OmniRoute (`auto` routing mode, with NVIDIA NIM as fallback).
...
### 6.4 Autonomous Agent Platform & Work Runtime (Aira 2.0 / v5.1)
...
- **Capabilities**:
  ...
  - Native Work Runtime Execution Engine (v5.1): Standalone background worker (`src/worker.ts`), database-backed atomic task leases, heartbeat extension, and `SELECT ... FOR UPDATE SKIP LOCKED` crash recovery.
  - Operational Note: Real customer Work Runtime E2E verification remains PENDING (Phase 3C). Temporary local Docker/Cloudflare tunnel infrastructure is PREVIEW ONLY, not production ready.
...
| **v5.1 Candidate Baseline** | 23472e97 | Verified clean on branch | PR #133 (OPEN / UNMERGED) |
| **Work Runtime Customer E2E** | Validated Run | PENDING | R2 Phase 3C |
| **Genuine Second-User Isolation** | Isolated Session | PENDING | Outstanding R2 Gate |
