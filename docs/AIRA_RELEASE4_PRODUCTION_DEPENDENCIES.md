...
| **PostgreSQL Database** | Non-production local/preview DB (`127.0.0.1:5432`) | Supabase PostgreSQL 17.6 Primary DB with Supavisor transaction pooler (port 6543) and direct port 5432 | `DATABASE_URL`, `DIRECT_URL` | 50+ pooled connections, pgvector enabled, `sslmode=verify-full` mandatory | Core platform offline | Point-in-time restore, backward-compatible migrations | Low |
| **Blob / Object Storage** | PostgreSQL `AgentArtifact.metadata` JSONB column + local scratch | Supabase S3-compatible Storage or AWS S3 | `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` | 100 GB+ durable object storage | Artifact downloads fail, fallback to database | Switch store adapter to internal database artifact | Low |
| **LLM Provider Gateway (OmniRoute)** | NVIDIA NIM (`meta/llama-3.2-11b-vision-instruct`) + OpenAI preview endpoints | Multi-provider router (NVIDIA NIM, OpenAI, Anthropic, Groq) | `NVIDIA_NIM_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `OMNIROUTE_API_KEY` | 100+ requests/sec, 1M+ tokens/day | Model reasoning degrades to fallback tier | Automatic OmniRoute provider failover | Medium |
