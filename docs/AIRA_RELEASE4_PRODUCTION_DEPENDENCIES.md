...
| **Blob / Object Storage** | PostgreSQL bytea (`AgentArtifact`) + local scratch | Supabase S3-compatible Storage or AWS S3 | `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` | 100 GB+ durable object storage | Artifact downloads fail, fallback to database | Switch store adapter to internal database artifact | Low |
