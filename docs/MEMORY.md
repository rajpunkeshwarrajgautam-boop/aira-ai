...
> **Active Development Branch**: `feat/aira-v5.1-work-runtime`  
> **Candidate Baseline SHA**: `23472e97bf028283ee9c25656ab1440fe4fdadf5`  
> **PR #133**: OPEN / UNMERGED  
...
- **Active Development Branch**: `feat/aira-v5.1-work-runtime` (Candidate SHA: `23472e97`, PR #133 OPEN / UNMERGED).
- **Working Tree**: Clean.
...
Execution of **R2 PHASE 3C — REAL CUSTOMER WORK RUNTIME E2E**:
1. Real customer end-to-end verification of the Work Runtime execution engine.
2. Genuine second-user tenant isolation verification across active database sessions.
3. Preservation of fail-closed security baselines (temporary local Docker/Cloudflare tunnel infrastructure is PREVIEW ONLY, not production ready).
...
2. **Multi-Provider AI Routing**: OmniRoute gateway (`nvidia/openai/gpt-oss-20b`) with circuit-breaker failover to NVIDIA NIM (`meta/llama-3.2-11b-vision-instruct`, verified production default, with OpenAI fallback) and OpenAI.
3. **Persistent Lexical Memory**: Scoped by `userId` in PostgreSQL via Prisma with full Row-Level Security.
4. **Knowledge Preview Callback**: Preview callback URL origin resolution without `AUTH_URL` dependency (`23472e97`).
5. **Work Runtime Engine (v5.1)**: Standalone background worker (`src/worker.ts`), atomic task leases, and crash recovery implemented; real customer E2E verification pending.
