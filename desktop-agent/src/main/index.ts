...
import { isTrustedIpcSource } from './ipc-security'
...
function assertTrustedIpcSender(event: IpcMainInvokeEvent): void {
  ...
  if (!contents || contents.isDestroyed() || !senderFrame || !mainFrame || !isTrustedIpcSource(
    { webContentsId: event.sender.id, processId: senderFrame.processId, routingId: senderFrame.routingId },
    { webContentsId: contents.id, processId: mainFrame.processId, routingId: mainFrame.routingId }
  )) throw new Error('Untrusted renderer IPC source.')
}

function trustedHandle<Args extends unknown[], Result>(channel: string, handler: (...args: Args) => Result | Promise<Result>): void {
...
app.whenReady().then(async()=>{
  ...
  trustedHandle('agent:send',async(request:unknown)=>runAgent(cleanAgentRequest(request),{approve:requestApproval}))
  trustedHandle('agent:approval-response',(id:string,approved:boolean)=>{const resolve=approvals.get(String(id));if(resolve){approvals.delete(String(id));resolve(Boolean(approved))}return true})
  trustedHandle('settings:get',()=>getSettings());trustedHandle('settings:set',(patch:unknown)=>setSettings((patch||{}) as Parameters<typeof setSettings>[0]));trustedHandle('secrets:status',()=>secretStatus());trustedHandle('secrets:set',(key:string,value:string)=>setSecret(String(key),String(value||'')))
  trustedHandle('ollama:status',()=>checkOllama(getSettings()));trustedHandle('compatible:status',()=>checkOpenAICompatible(getSettings()));trustedHandle('ollama:pull',(model:string)=>pullOllamaModel(getSettings(),String(model||'').slice(0,200)))
  trustedHandle('voice:listen',()=>listenOnce());trustedHandle('voice:speak',(text:string)=>speak(String(text||'').slice(0,12_000)));trustedHandle('voice:stop-speaking',()=>stopSpeaking());trustedHandle('voice:start-continuous',()=>startContinuous(transcript=>{const wake=getSettings().wakeWord.toLowerCase().trim();const lower=transcript.toLowerCase();if(wake&&!lower.includes(wake))return;const idx=wake?lower.indexOf(wake):-1;const text=idx>=0?transcript.slice(idx+wake.length).replace(/^[,:\s-]+/,'').trim():transcript;mainWindow?.webContents.send('voice:transcript',{text,raw:transcript})}));trustedHandle('voice:stop-continuous',()=>stopContinuous())
  trustedHandle('memory:list',(limit?:number)=>listMemories(Number(limit)||100));trustedHandle('memory:delete',(id:string)=>deleteMemory(String(id)));trustedHandle('rag:status',()=>ragStatus());trustedHandle('rag:index',()=>indexWorkspace(getSettings()));trustedHandle('tasks:list',()=>listTasks());trustedHandle('audit:list',(limit?:number)=>getAudit(Number(limit)||100))
  trustedHandle('update:check',()=>checkForUpdates());trustedHandle('update:install',()=>installUpdate());trustedHandle('app:version',()=>app.getVersion())
