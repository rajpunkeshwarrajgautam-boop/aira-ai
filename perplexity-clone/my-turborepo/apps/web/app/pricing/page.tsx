					...
					{PLANS.map((plan) => {
						...
						return (
							<section key={plan.name} className={cn("aira-premium-card aira-card-hover relative flex flex-col overflow-hidden rounded-[28px] p-6", plan.highlight && "aira-pro-glow border-accent/25 md:-translate-y-2")}>
								...
								{plan.name === "Free" ? <Button variant="outline" disabled className="relative h-11 w-full rounded-xl bg-surface-inset/60">{plan.buttonText}</Button> : <Button asChild className={cn("relative h-11 w-full rounded-xl shadow-sm", plan.highlight ? "bg-[linear-gradient(135deg,hsl(var(--accent)),hsl(var(--accent-violet)))] hover:opacity-95" : "bg-content-primary hover:bg-content-primary/90")}><Link href="/upgrade">{plan.buttonText}</Link></Button>}
							</section>
						);
					})}
				</div>
