...
export async function GET(): Promise<Response> {
  ...
  if (process.env.MULTIMODAL_INGESTION_ENABLED !== "true") {
    return Response.json({ error: { code: "MULTIMODAL_INGESTION_DISABLED", message: "Uploaded knowledge ingestion is not enabled on this deployment." } }, { status: 503 });
  }
  const rows = await listKnowledgeAssets(session.user.id, 50);
