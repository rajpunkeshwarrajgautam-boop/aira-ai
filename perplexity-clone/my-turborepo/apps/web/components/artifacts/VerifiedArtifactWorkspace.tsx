...
import { Download, FilePlus2, FileText, Layers, Loader2, RefreshCw, ShieldCheck } from "lucide-react";
    ...
    <main className="min-h-[calc(100dvh-58px)] bg-[var(--aira-canvas,#F9F8F6)] px-4 py-6 text-[#111115] md:px-7">
      <div className="mx-auto max-w-[1500px] space-y-5">
        <header className="flex flex-wrap items-end justify-between gap-3">
          ...
          <button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-xl border border-[rgba(17,17,21,0.08)] bg-white px-3.5 py-2 text-xs font-semibold text-[#111115] shadow-2xs transition hover:bg-[#FAF9F6]">
        ...
        <section className="grid gap-4 xl:grid-cols-[330px_minmax(0,1fr)]">
          <aside className="space-y-4">
            <div className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white p-5 shadow-xs">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} className="mt-3 w-full rounded-xl border border-[rgba(17,17,21,0.12)] bg-[#FAF9F6] px-3.5 py-2 text-sm text-[#111115] placeholder:text-[#8F8E98] outline-none focus:border-[#3A0CA3]" />
              <select value={format} onChange={(e) => setFormat(e.target.value as typeof format)} className="mt-2.5 w-full rounded-xl border border-[rgba(17,17,21,0.12)] bg-[#FAF9F6] px-3.5 py-2 text-xs font-medium text-[#111115] outline-none focus:border-[#3A0CA3]">
                <option value="MARKDOWN">Markdown</option>
                <option value="TXT">Plain text</option>
                <option value="JSON">JSON</option>
                <option value="CSV">CSV</option>
              </select>
              <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={6} className="mt-2.5 w-full resize-none rounded-xl border border-[rgba(17,17,21,0.12)] bg-[#FAF9F6] px-3.5 py-2 font-mono text-xs leading-5 text-[#111115] placeholder:text-[#8F8E98] outline-none focus:border-[#3A0CA3]" />
              <button type="button" onClick={() => void createArtifact()} disabled={busy || !name.trim() || !content.trim()} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#3A0CA3] hover:bg-[#2D0A82] px-3.5 py-2.5 text-sm font-medium text-white shadow-xs transition disabled:cursor-not-allowed disabled:border disabled:border-[rgba(17,17,21,0.08)] disabled:bg-[rgba(17,17,21,0.04)] disabled:text-[#8F8E98]">
            ...
            <div className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white p-4 shadow-xs">
              <p className="px-1 py-1 text-xs font-medium text-[#6B6A75]">Artifact library ({artifacts.length})</p>
              {loading ? (
                <div className="grid place-items-center py-10"><Loader2 className="size-4 animate-spin text-[#3A0CA3]" /></div>
              ) : artifacts.length ? (
                <div className="mt-2 max-h-[520px] space-y-1 overflow-auto">
                  {artifacts.map((artifact) => (
                    <button key={artifact.id} type="button" onClick={() => setSelectedId(artifact.id)} className={`w-full rounded-xl px-3 py-3 text-left transition ${artifact.id === selectedId ? "border border-[#3A0CA3] bg-[#FAF9F6] shadow-2xs" : "border border-transparent hover:bg-[#FAF9F6] text-[#111115]"}`}>
                      <p className="truncate text-sm font-medium">{artifact.name}</p>
                      <p className="mt-1 text-xs text-[#6B6A75]">{artifact.format} · v{artifact.currentVersion}</p>
                    </button>
                  ))}
                </div>
              ) : <p className="px-3 py-8 text-center text-sm text-[#8F8E98]">No artifacts yet.</p>}
            </div>
          </aside>
          <div className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white p-5 shadow-xs">
            {!selected ? (
              <div className="grid min-h-[420px] place-items-center p-8 text-center">
                <div>
                  ...
                  <p className="mt-4 text-sm font-semibold text-[#111115]">No artifact selected</p>
                  <p className="mx-auto mt-1 max-w-sm text-xs leading-5 text-[#6B6A75]">
                    Select an artifact from the library or create a new durable deliverable with verified checksums.
                  </p>
                </div>
              </div>
            ) : (
              <>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <a href={`/api/artifacts/${encodeURIComponent(selected.id)}/download?version=${selected.currentVersion}`} className="inline-flex items-center gap-2 rounded-xl bg-[#3A0CA3] hover:bg-[#2D0A82] px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition">
                ...
                {activeVersion ? (
                  <div className="mt-5 grid gap-3 sm:grid-cols-3">
                    <div className="rounded-xl border border-[rgba(17,17,21,0.06)] bg-[#FAF9F6] p-3">
                      <p className="text-xs text-[#6B6A75]">Checksum</p>
                      <p className="mt-1 break-all font-mono text-[11px] text-[#111115]">{activeVersion.checksum}</p>
                    </div>
                    <div className="rounded-xl border border-[rgba(17,17,21,0.06)] bg-[#FAF9F6] p-3">
                      <p className="text-xs text-[#6B6A75]">Size</p>
                      <p className="mt-1 text-sm font-semibold text-[#111115]">{activeVersion.sizeBytes.toLocaleString()} bytes</p>
                    </div>
                    <div className="rounded-xl border border-[rgba(17,17,21,0.06)] bg-[#FAF9F6] p-3">
                      <p className="text-xs text-[#6B6A75]">Validation</p>
