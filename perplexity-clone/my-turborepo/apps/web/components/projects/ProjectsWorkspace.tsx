  ...
  return (
    <main className="min-h-[calc(100dvh-58px)] bg-[var(--aira-canvas,#F9F8F6)] px-4 py-6 text-[#111115] md:px-7">
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#3A0CA3]">Projects</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-[#111115]">Persistent mission context</h1><p className="mt-1 text-sm text-[#6B6A75]">Projects, managed runs and project-scoped artifacts are loaded from authenticated server APIs.</p></div><button type="button" onClick={() => void loadProjects()} className="inline-flex items-center gap-2 rounded-lg border border-[rgba(17,17,21,0.1)] bg-white px-3 py-2 text-xs text-[#111115] shadow-xs transition hover:bg-[#FAF9F6]"><RefreshCw className="size-3.5" />Refresh</button></div>
        ...
        <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white p-4 shadow-xs">
            <h2 className="text-sm font-semibold text-[#111115]">Create project</h2>
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Project name" className="mt-3 w-full rounded-lg border border-[rgba(17,17,21,0.12)] bg-white px-3 py-2 text-sm text-[#111115] placeholder:text-[#8F8E98] outline-none focus:border-[#3A0CA3]" />
            <textarea value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="Objective" rows={3} className="mt-2 w-full rounded-lg border border-[rgba(17,17,21,0.12)] bg-white px-3 py-2 text-sm text-[#111115] placeholder:text-[#8F8E98] outline-none focus:border-[#3A0CA3]" />
            <button type="button" onClick={() => void createProject()} disabled={busy !== null || name.trim().length < 2 || objective.trim().length < 3} className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#3A0CA3] px-3 py-2 text-sm font-semibold text-white shadow-xs transition hover:bg-[#2D0A82] disabled:opacity-40">{busy === "create" ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />}Create</button>
            <div className="mt-5 border-t border-[rgba(17,17,21,0.08)] pt-3"><p className="mb-2 text-[10px] uppercase tracking-[0.14em] text-[#6B6A75]">Active projects · {projects.length}</p><div className="max-h-[540px] space-y-1 overflow-auto">{projects.map((project) => <button key={project.id} type="button" onClick={() => setSelectedId(project.id)} className={`w-full rounded-xl px-3 py-3 text-left transition ${project.id === selectedId ? "bg-[#F3F1EC] text-[#111115]" : "hover:bg-[#FAF9F6] text-[#111115]"}`}><p className="truncate text-sm font-medium text-[#111115]">{project.name}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-[#6B6A75]">{project.objective}</p></button>)}</div></div>
          </aside>
          <section className="space-y-4">
            {!selected ? <div className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white px-6 py-20 text-center text-sm text-[#6B6A75] shadow-xs">Create a project to begin.</div> : <>
              <div className="rounded-2xl border border-[rgba(17,17,21,0.08)] bg-white p-5 shadow-xs"><div className="flex items-center gap-2"><Pencil className="size-4 text-[#3A0CA3]" /><h2 className="text-sm font-semibold text-[#111115]">Project definition</h2></div><input value={editName} onChange={(e) => setEditName(e.target.value)} className="mt-4 w-full rounded-lg border border-[rgba(17,17,21,0.12)] bg-white px-3 py-2 text-sm text-[#111115] outline-none focus:border-[#3A0CA3]" /><textarea value={editObjective} onChange={(e) => setEditObjective(e.target.value)} rows={4} className="mt-2 w-full rounded-lg border border-[rgba(17,17,21,0.12)] bg-white px-3 py-2 text-sm text-[#111115] outline-none focus:border-[#3A0CA3]" /><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void saveProject()} disabled={busy !== null} className="inline-flex items-center gap-2 rounded-lg border border-[rgba(17,17,21,0.12)] bg-white px-3 py-2 text-xs text-[#111115] shadow-xs hover:bg-[#FAF9F6]"><Save className="size-3.5" />Save changes</button><button type="button" onClick={() => void archiveProject()} disabled={busy !== null} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-600 hover:bg-red-100"><Archive className="size-3.5" />Archive</button><Link href={`/build?project=${encodeURIComponent(selected.id)}`} className="inline-flex items-center gap-2 rounded-lg bg-[#3A0CA3] px-3 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-[#2D0A82]"><Boxes className="size-3.5" />Open in Build</Link></div></div>
