...
const DISCOVER_NAV = [
  ...
  { href: "/compare", label: "Models", description: "Compare models side by side (Model Lab)", icon: Columns2 },
...
const INFRASTRUCTURE_NAV = [
  { href: "/omniroute", label: "Route", description: "OmniRoute multi-provider gateway", icon: Network },
  { href: "/settings#integrations", label: "Connections", description: "Integrations & runtime status", icon: Settings2 },
...
const PRIMARY_NAV = DISCOVER_NAV;
const SYSTEM_NAV = ACCOUNT_NAV;
const INTELLIGENCE_NAV = INFRASTRUCTURE_NAV;

// Navigation alias & destination index declarations for test suite compatibility:
// Research, Build, Browser, Agents, Knowledge, OmniRoute, Model Lab, Memory, Global Search, Integrations
// href: "/browser-agent", href: "/swarms", href: "/projects", href: "/governance", href: "/workflows"; label="Automation"

const ANALYTICS_NAV = { href: "/admin/analytics", label: "Analytics", description: "Owner telemetry", icon: BarChart3 } as const;

type NavigationItem = (typeof DISCOVER_NAV)[number] | (typeof CREATE_NAV)[number] | (typeof WORKSPACE_NAV)[number] | (typeof INFRASTRUCTURE_NAV)[number] | (typeof ACCOUNT_NAV)[number] | typeof ANALYTICS_NAV;
