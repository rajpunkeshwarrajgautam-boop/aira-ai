			...
			<div className="aira-enter group flex gap-3 py-5">
				...
				<div className="aira-assistant-response min-w-0 flex-1 border-b border-white/[0.07] pb-5">
					<div className="mb-2 flex min-h-7 items-center justify-between gap-3">
						<div className="flex min-w-0 items-center gap-2"><p className="text-[12px] font-semibold text-content-primary">AIRA AI</p>{effectiveCitations.length > 0 ? <span className="inline-flex items-center gap-1 text-[10px] text-content-tertiary"><Globe2 className="size-3" aria-hidden /> {effectiveCitations.length} sources</span> : null}</div>
		...
		<div className="aira-thread-layout w-full">
			<div className="aira-thread-toolbar sticky top-0 z-20 flex h-16 items-center justify-between gap-3 border-b border-white/[0.07] bg-[#0b1020]/92 px-4 backdrop-blur-xl sm:px-5">
				...
				<div className="hidden items-center rounded-xl border border-white/[0.08] bg-[#0d1322] p-1 sm:flex" aria-label="Research tools">
					<span className="rounded-lg bg-accent/15 px-4 py-1.5 text-[10px] font-semibold text-accent" title="AIRA standard research">AIRA</span>
			...
			<div className="aira-thread-columns grid min-h-0 grid-cols-1 xl:grid-cols-[minmax(0,1fr)_280px]">
					...
					<div className="aira-message-stack mx-auto max-w-[960px]">
						{showEmptyHint ? (
							<div className="aira-enter py-8 sm:py-12">
								<div className="mx-auto flex max-w-xl flex-col items-center text-center"><div className="mx-auto flex size-10 items-center justify-center rounded-xl border border-accent/20 bg-accent/10 text-accent"><Sparkles className="size-5" strokeWidth={1.6} aria-hidden /></div><h2 className="mt-4 text-2xl font-semibold tracking-[-0.03em] text-content-primary">What are you working on?</h2><p className="mx-auto mt-2 max-w-md text-[12px] leading-5 text-content-tertiary">Research the web, work with private files, compare models, or hand a longer workflow to an agent.</p></div>
								{exampleQueries.length > 0 && onPickExample ? <div className="mx-auto mt-7 grid max-w-2xl gap-2 sm:grid-cols-3">{exampleQueries.map((item) => <button key={item} type="button" onClick={() => onPickExample(item)} className="min-h-20 rounded-xl border border-white/[0.08] bg-[#101727]/75 px-3 py-3 text-left text-[11px] leading-5 text-content-secondary transition hover:border-accent/25 hover:bg-[#141c30] hover:text-content-primary"><span className="mb-1 block text-[10px] font-semibold text-accent/80">Ask AIRA</span>{item}</button>)}</div> : null}
								<div className="mx-auto mt-3 grid max-w-2xl gap-1 sm:grid-cols-3">{STARTERS.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} className="group flex items-start gap-2.5 rounded-lg px-2.5 py-2 text-left transition hover:bg-white/[0.035]"><Icon className="mt-0.5 size-3.5 shrink-0 text-content-tertiary transition group-hover:text-accent" strokeWidth={1.7} aria-hidden /><span><strong className="block text-[11px] font-medium text-content-secondary group-hover:text-content-primary">{item.title}</strong><small className="mt-0.5 block text-[10px] leading-4 text-content-tertiary">{item.description}</small></span></Link>; })}</div>
							</div>
						) : null}

						{messages.map((message) => message.role === "USER" ? (
							<div key={message.id} className="aira-enter group flex w-full justify-end py-3"><div className="max-w-[86%] sm:max-w-[78%]"><div className="rounded-2xl border border-accent/20 bg-accent/15 px-4 py-3 text-[14px] leading-6 text-blue-50">{message.content}</div><div className="mt-1 flex justify-end gap-1 opacity-0 transition group-hover:opacity-100 focus-within:opacity-100"><ReusePromptButton text={message.content} /><MessageCopyButton text={message.content} /></div></div></div>
						) : <div key={message.id}>{renderAssistant({ content: message.content, citations: message.citations, streaming: false })}</div>)}

						{streamingUserQuery ? <div className="flex w-full justify-end py-3" aria-label="Streaming user message"><div className="max-w-[86%] rounded-2xl border border-accent/20 bg-accent/15 px-4 py-3 text-[14px] leading-6 text-blue-50 sm:max-w-[78%]">{streamingUserQuery}</div></div> : null}
				...
				<aside className="aira-live-inspector hidden border-l border-white/[0.07] bg-[#0d1320]/72 p-3 xl:block" aria-label="Conversation inspector">
					<div className="sticky top-20 space-y-3">
						<section className="aira-inspector-section border-b border-white/[0.07] pb-3"><p className="text-[11px] font-semibold text-content-primary">Routing</p><Link href="/omniroute" className="mt-2 flex items-center justify-between rounded-lg px-1 py-2 transition hover:bg-white/[0.035]"><span><strong className="block text-[11px] font-semibold text-content-primary">AIRA Auto</strong><small className="mt-0.5 block text-[9px] text-content-tertiary">Provider routing follows workspace policy</small></span><Network className="size-4 text-accent" strokeWidth={1.6} /></Link></section>

						<section className="aira-inspector-section border-b border-white/[0.07] pb-3"><div className="flex items-center justify-between"><p className="text-[11px] font-semibold text-content-primary">Sources</p><span className="text-[10px] tabular-nums text-content-tertiary">{inspectorCitations.length || 0}</span></div><div className="mt-2 space-y-1">{inspectorCitations.length === 0 ? <p className="border-t border-dashed border-white/[0.07] py-3 text-[10px] leading-4 text-content-tertiary">Sources appear here when AIRA grounds an answer on the web.</p> : inspectorCitations.slice(0, 6).map((citation) => <a key={`${citation.index}-${citation.url}`} href={citation.url} target="_blank" rel="noreferrer" className="group flex gap-2 rounded-lg px-1 py-2 transition hover:bg-white/[0.035]"><span className="w-4 shrink-0 text-[10px] text-accent">{citation.index}</span><span className="min-w-0 flex-1"><strong className="block truncate text-[10px] font-medium text-content-secondary group-hover:text-content-primary">{citation.title}</strong><small className="mt-0.5 flex items-center gap-1 truncate text-[9px] text-content-tertiary">{hostnameFromUrl(citation.url)}<ExternalLink className="size-2.5" aria-hidden /></small></span></a>)}</div></section>

						<section className="aira-inspector-section border-b border-white/[0.07] pb-3"><p className="text-[11px] font-semibold text-content-primary">Conversation</p><dl className="mt-2 space-y-2 text-[10px]"><div className="flex items-center justify-between gap-3"><dt className="text-content-tertiary">Created</dt><dd className="truncate text-content-secondary">{createdAt && !Number.isNaN(createdAt.getTime()) ? new Intl.DateTimeFormat(undefined, { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }).format(createdAt) : "New thread"}</dd></div><div className="flex items-center justify-between"><dt className="text-content-tertiary">Messages</dt><dd className="tabular-nums text-content-secondary">{messages.length}</dd></div><div className="flex items-center justify-between"><dt className="text-content-tertiary">Sources</dt><dd className="tabular-nums text-content-secondary">{inspectorCitations.length}</dd></div><div className="flex items-center justify-between"><dt className="text-content-tertiary">State</dt><dd className="inline-flex items-center gap-1 text-content-secondary"><span className="size-1.5 rounded-full bg-accent" />Ready</dd></div></dl></section>

						<section className="aira-inspector-section"><p className="text-[11px] font-semibold text-content-primary">Actions</p><div className="mt-2 space-y-0.5"><button type="button" disabled={!copyAll} onClick={() => void navigator.clipboard.writeText(copyAll)} className="flex min-h-9 w-full items-center gap-2 rounded-lg px-1 py-2 text-left text-[10px] text-content-secondary transition hover:bg-white/[0.035] hover:text-content-primary disabled:opacity-40"><Copy className="size-3.5" strokeWidth={1.6} />Copy conversation</button><button type="button" disabled={!shareableText.trim()} onClick={() => void shareConversation()} className="flex min-h-9 w-full items-center gap-2 rounded-lg px-1 py-2 text-left text-[10px] text-content-secondary transition hover:bg-white/[0.035] hover:text-content-primary disabled:cursor-not-allowed disabled:opacity-40"><Share2 className="size-3.5" strokeWidth={1.6} />Share conversation</button><button type="button" onClick={() => window.print()} className="flex min-h-9 w-full items-center gap-2 rounded-lg px-1 py-2 text-left text-[10px] text-content-secondary transition hover:bg-white/[0.035] hover:text-content-primary"><FileDown className="size-3.5" strokeWidth={1.6} />Print / save as PDF</button><button type="button" onClick={() => emitComposerCommand("/new")} className="flex min-h-9 w-full items-center gap-2 rounded-lg px-1 py-2 text-left text-[10px] text-content-secondary transition hover:bg-white/[0.035] hover:text-content-primary"><MessageSquarePlus className="size-3.5" strokeWidth={1.6} />New conversation</button></div>{shareFeedback ? <p className="mt-2 rounded-lg bg-white/[0.035] px-2 py-1.5 text-[10px] text-content-tertiary" role="status">{shareFeedback}</p> : null}</section>
					</div>
				</aside>
			</div>
		</div>
