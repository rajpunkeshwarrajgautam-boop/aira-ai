					...
					<section className="space-y-4" aria-labelledby="new-agent-task-heading">
						<div className="aira-card aira-agent-objective aira-fun-card rounded-3xl p-5 sm:p-6">
							<div className="relative z-[1] flex items-center justify-between gap-3">
								...
								<span className={cn("aira-status-pill rounded-full py-1 pl-5 pr-2.5 text-[11px] font-semibold", feature?.enabled && feature.configured ? "bg-emerald-500/10 text-emerald-700" : "bg-amber-500/10 text-amber-800")}>{feature?.enabled && feature.configured ? "Runtime ready" : "Runtime offline"}</span>
							</div>
							...
							<div className="relative z-[1] mt-2 flex items-center justify-between text-xs text-content-tertiary"><span>{canSubmit ? "Aira will verify completion before reporting done." : "Complete the requirements below to enable tasks."}</span><span>{objective.length.toLocaleString()}/4,000</span></div>
