...
def validate(catalog: dict[str, Any]) -> list[str]:
    ...
    for index, source in enumerate(sources):
        ...
        if decision == "candidate_filtered":
            ...
            if approved:
                for field in ("license_review", "provenance_review", "contamination_review", "context_fit_review"):
...
def self_test() -> dict[str, Any]:
    ...
    bad["sources"][1]["approved_for_training"] = True
    if not validate(bad):
        raise RuntimeError("excluded training source was not rejected")
    return {"status": "PASS", "contract": "core-v0-source-decisions"}
